...
Forms will use Web3Forms directly from the browser. No form or form-receiver backend was added in this batch. Analytics is inactive: Cookie Settings explains that state and does not store a fictitious choice. The imported preview batch used `noindex, nofollow` without a sitemap. This describes that historical batch, not the current formal-release target. Formal public pages must meet the approved indexability requirements; preview restrictions must be identified and removed for the authorized release. This documentation change does not claim that robots, sitemap or deployed output have already been updated.
