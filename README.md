# Nine-page static site

Next.js static export for `/`, `/products/`, `/products/m-350/`, `/applications/`, and five application-detail routes for Coatings, Plastics, Masterbatch, Printing Inks and Paper. No WordPress, database, API server, secrets or remote content are required to build or serve it. Content belongs to D23; D32 implements and publishes approved changes.
...
Open `http://127.0.0.1:8333/` and navigate through Products or Applications to the exported routes. Stop the preview with Ctrl+C before running tests: the test runner owns the same loopback port and refuses to use an unrelated server. `npm test` requires installed Google Chrome and tests the built `out/` directory, not the development server. Run `npm run build` after changing content or code.
...
- `content/home.json`, `products.json`, `m350.json`, `applications.json`, `chrome.json` and `content/application-details/*.md`: content and shared navigation; all committed with the code.
...
Nine pages are implemented. Shared navigation and Home retain approved links to future destinations; these are genuine missing-page dependencies. Products exposes only M350 as a working detail link and exposes Applications as a ready support route. Applications links M350 in its four approved relationship occurrences, keeps the other twenty-six grade occurrences as plain text, opens the five implemented child-Application routes, shows only Products in procurement continuation, and omits its contextual RFQ actions. The child pages use the same readiness boundary: only M350, Products and the implemented Plastics/Masterbatch cross-links are active; unavailable Grade, Document, Sample and contextual RFQ controls remain non-interactive. Specialty Materials still has no child route. The fixed shared RFQ link remains per the approved chrome contract.

Forms will use Web3Forms directly from the browser. No form or form-receiver backend was added in this batch. Analytics is inactive: Cookie Settings explains that state and does not store a fictitious choice. The original four routes remain `noindex, nofollow`. The five application-detail pages are `index, follow`, use exact production canonicals and are the only URLs in the generated sitemap; `robots.txt` allows crawling and advertises that sitemap. This technical configuration does not claim that a search engine has indexed the pages.
