# Twenty-five-route static site

Next.js static export for `/`, `/products/`, fourteen Product Detail routes, `/applications/`, five application-detail routes, `/request-a-quote/`, `/thank-you/`, and `/privacy-policy/`. There is no WordPress runtime, database, custom API server or form backend. Content belongs to D23; D32 implements and publishes approved changes.
...
- `content/home.json`, `products.json`, `m350.json`, `applications.json`, `rfq.json`, `thank-you.json`, `privacy.json`, `chrome.json`, `content/product-details/*.json`, and `content/application-details/*.md`: approved content and shared navigation; all committed with the code.
- `app/`: fixed routes, the validated static Product Detail route, application-detail routes, and metadata.
...
Twenty-five routes are implemented. Shared navigation and Home retain approved links to future destinations; these are genuine missing-page dependencies. Products exposes all fourteen Product Detail routes and Applications as a ready support route. Applications keeps its approved readiness boundary: it links M350 in its four relationship occurrences, keeps the other twenty-six grade occurrences as plain text, opens the five implemented child-Application routes, shows Products in procurement continuation, and exposes the ready RFQ actions. The child pages activate M350, Products, the implemented Plastics/Masterbatch cross-links, and RFQ; unavailable Document and Sample controls remain non-interactive. Specialty Materials still has no child route. `/request-sample/`, `/request-documents/`, `/privacy-policy-bm/` and `/cookie-policy/` remain unresolved routes and are not represented as completed pages or successful workflows.

RFQ submissions go directly from the browser to Web3Forms. The site accepts success only from an explicit provider response, stores a short-lived one-time session receipt, and lets `/thank-you/` display success only when the URL and receipt agree. Rejection, rate limiting, timeout, malformed response and network failure retain the form without creating success. No form-receiver backend was added. Analytics is inactive: Cookie Settings explains that state and does not store a fictitious choice. All twenty-five content routes are `index, follow`, use exact production canonicals and are included in the generated sitemap; `robots.txt` allows crawling and advertises that sitemap. This technical configuration makes the pages eligible for discovery but does not claim that Google has indexed them.
