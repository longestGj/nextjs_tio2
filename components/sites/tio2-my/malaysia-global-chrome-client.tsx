...
import type { MalaysiaGlobalChromePublicProjection } from "@/lib/content/tio2-my-global-chrome-public";

import styles from "./malaysia-global-chrome.module.css";
import {
  MalaysiaCookieSettingsHost,
  MalaysiaCookieSettingsTrigger,
} from "./consent/malaysia-cookie-settings";
...
function RfqLink({
  ...
  return (
    <MalaysiaPrivateRfqLink className={className} href={href} onClick={onClick}>
      {children}
...
export function MalaysiaGlobalHeaderClient({
  ...
  useEffect(() => {
    ...
    if (inlineMobileMenu) {
      ...
      const background = [
        ...
        header.querySelector(`.${styles.logoLink}`),
        header.querySelector(`.${styles.headerRfq}`),
      ]
      ...
      return () => {
        ...
        const desktopCurrent = header.querySelector<HTMLElement>(
          `.${styles.desktopNav} [aria-current="page"]`,
        );
  ...
  return (
    <header
      ref={headerRef}
      className={`${styles.header} ${inlineMobileMenu ? styles.inlineMobileHeader : ""}`}
      lang="en"
    >
      ...
      {inlineMobileMenu ? (
        ...
        >
          <div className={styles.menuTopbar}>
            ...
            />
            <RfqLink
              className={styles.headerRfq}
              href={chrome.rfq.href}
              onClick={() => setOpen(false)}
            >
              {chrome.rfq.compactLabel}
            </RfqLink>
            <button
              ref={closeRef}
              type="button"
...
export function MalaysiaGlobalFooterClient({
  ...
  return (
    <>
      <footer
        className={`${styles.footer} ${inlineMobileMenu ? styles.inlineMobileFooter : ""}`}
        lang="en"
      >
        <div className={styles.footerGrid}>
          ...
          <nav aria-label="Footer explore navigation">
            ...
            {chrome.footer.explore.map((item) => (
          ...
          <nav aria-label="Footer information navigation">
            ...
            {chrome.footer.information.map((item) => (
              <a key={item.href} href={item.href}>
                {item.label}
              </a>
            ))}
        ...
        <nav
          className={styles.legalUtilities}
          aria-label="Legal and privacy navigation"
        >
          {chrome.footer.legalUtilities.map((item) =>
            item.href === null ? (
              <MalaysiaCookieSettingsTrigger key={item.label}>
                {item.label}
              </MalaysiaCookieSettingsTrigger>
            ) : (
