...
export interface MalaysiaGlobalChromePublicProjection {
  ...
  readonly footer: {
    ...
    readonly legalUtilities: readonly {
      readonly label: string;
...
export function projectMalaysiaGlobalChrome(
  chrome: Tio2MyGlobalChrome,
): MalaysiaGlobalChromePublicProjection {
  const navigation = chrome.navigation.map(({ label, href }) => ({
    label,
    href,
  }));
  const byId = new Map(
  ...
  return {
    logo: {
      primary: publicLogo(chrome.logo.primary),
      reverse: publicLogo(chrome.logo.reverse),
    },
    navigation,
    rfq: {
      label: chrome.rfq.label,
      compactLabel: chrome.rfq.compactLabel,
    ...
    footer: {
      ...
      information: chrome.footer.information.flatMap(
        (id) => byId.get(id) ?? [],
      ),
      legalUtilities: chrome.footer.legalUtilities.map(({ label, href }) => ({
        label,
        href,
      })),
